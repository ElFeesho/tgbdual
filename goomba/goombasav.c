...
#define goomba_error(...) \
    { sprintf(last_error, __VA_ARGS__); }
...
const char *stateheader_summary_str(const stateheader *sh) {
    sprintf(goomba_strbuf, "%s: %s (%u b / %u uncomp)", stateheader_typestr(
                                                            F16(sh->type)),
            sh->title, F16(sh->size), F32(sh->uncompressed_size));
    return goomba_strbuf;
}
...
stateheader *stateheader_for(const void *gba_data, const char *gbc_title) {
    ...
    free(headers);
    if (use_this == NULL)
        sprintf(last_error, "Could not find SRAM data for %s", title);
    return use_this;
}
