...
void printScanResults(std::vector<ptrdiff_t> &results, tgbdual &tgb) {
    ...
    for (ptrdiff_t &value : results) {
        std::stringstream s;
        s << std::hex << value;
        tgb.addConsoleOutput(std::to_string(++i) + ": 0x" + s.str());
    }
}
...
void registerScanCommands(tgbdual &tgb) {
    ...
    tgb.addConsoleCommand("scan", [&](std::vector<std::string> args) {
        ...
        tgb.addConsoleOutput("Found " + std::to_string((int) resultCount) + " results");
    });
    ...
    tgb.addConsoleCommand("search_greater", [&](std::vector<std::string> args) {
        ...
        tgb.addConsoleOutput("Total values: " + std::to_string(resultCount));
    });
    ...
    tgb.addConsoleCommand("scan_threshold", [&](std::vector<std::string> args) {
        if (args.size() == 0) {
            tgb.addConsoleOutput("Scan threshold: " + std::to_string(scanEngine.scan_threshold()));
        } else {
            scanEngine.set_scan_threshold(console_cmd::toInt<size_t>(args[0]));
            tgb.addConsoleOutput("Scan threshold now: " + std::to_string(scanEngine.scan_threshold()));
        }
    });
