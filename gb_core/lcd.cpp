...
void lcd::bg_render(void *buf, int scanline) {
    ...
    byte tile;
    int i, x, y;
    byte *vrams[2] = {ref_gb->get_cpu()->get_vram(),
...
void lcd::win_render(void *buf, int scanline) {
    ...
    word *now_share =
        (word *)(ref_gb->get_cpu()->get_vram() + share + ((y & 7) << 1));
    word *now_pat =
        (word *)(ref_gb->get_cpu()->get_vram() + pat + ((y & 7) << 1));
    dword tmp_dat;
    dword calc1, calc2;
...
void lcd::sprite_render(void *buf, int scanline) {
    ...
    word l1, l2, tmp_dat;
    word pal[2][4], *cur_p;
    byte *oam = ref_gb->get_cpu()->get_oam(),
    ...
    for (i = 39; i >= 0; i--) {
        ...
        if (sp_size) { // 8*16
            ...
            if (scanline - y + 15 < 8) {
                ...
                tmp_dat = *(word *)(vram + (tile & 0xfe) * 16 + now * 2 +
                                    ((atr & 0x40) ? 16 : 0));
            } else {
                now = (atr & 0x40) ? ((y - scanline) & 7) : (7 - (y - scanline) & 7);
                tmp_dat = *(word *)(vram + (tile & 0xfe) * 16 + now * 2 +
    ...
    for (i = 39; i >= 0; i--) {
        ...
        if (sp_size) { // 8*16
            ...
            if (scanline - y + 15 < 8) { //上半分
                now = (atr & 0x40) ? ((y - scanline) & 7) : (7 - (y - scanline) & 7);
                tmp_dat = *(word *)(vram + bank + (tile & 0xfe) * 16 + now * 2 +
                                    ((atr & 0x40) ? 16 : 0));
            } else { // 下半分
                now = (atr & 0x40) ? ((y - scanline) & 7) : (7 - (y - scanline) & 7);
                tmp_dat = *(word *)(vram + bank + (tile & 0xfe) * 16 + now * 2 +
