...
public:
    ...
    lua_script_vm vm{&scriptServices};
};
...
TEST_F(LuaVMTest, LuaVM_CanIndicatedInabilityToHandleUnhandledCommands) {
    vm.loadScript("function handleCommand(command, args)\n"
                          "   return false\n"
                          "end\n");
...
TEST_F(LuaVMTest, LuaVM_CanRegisterConsoleCommands) {
    vm.loadScript(
            "GameBoy.register_console_command(\"cfunc\", function(args)\n"
            "    GameBoy.print(\"Expected Func \"..args[1])\n"
            "end)");
