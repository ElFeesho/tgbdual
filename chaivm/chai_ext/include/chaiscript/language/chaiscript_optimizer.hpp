...
namespace chaiscript {
    namespace optimizer {
        ...
        auto make_compiled_node(const eval::AST_Node_Impl_Ptr <T> &original_node, std::vector<eval::AST_Node_Impl_Ptr < T>>

        children,
        Callable callable
        ) {
        return chaiscript::make_shared<eval::AST_Node_Impl < T>, eval::Compiled_AST_Node <T>>(original_node,
        std::move(children), std::move(callable)
        );
    }
        ...
        auto optimize(const eval::AST_Node_Impl_Ptr <T> &node) {
            if (node->identifier == AST_Node_Type::Block) {
                ...
                if (keepers.size() == num_children) {
                    ...
                    std::vector<eval::AST_Node_Impl_Ptr < T>>
                    new_children;
                ...
                if (child->identifier == AST_Node_Type::Block
                    ...
                    for (size_t i = 0; i < num_sub_children; ++i) {
                        ...
                        if (sub_child->identifier == AST_Node_Type::Fun_Call) {
                            child->children[i] = chaiscript::make_shared<eval::AST_Node_Impl < T>, eval::Unused_Return_Fun_Call_AST_Node <
                                                                                                   T >> (sub_child->text, sub_child->location, std::move(sub_child->children));
                        }
                    }
                }
            }
    ...
    struct If {
        ...
        auto optimize(const eval::AST_Node_Impl_Ptr <T> &node) {
            if ((node->identifier == AST_Node_Type::If)
                ...
                const auto condition = std::dynamic_pointer_cast<eval::Constant_AST_Node < T>>
                (node->children[0])->m_value;
            ...
            if (node->identifier == AST_Node_Type::Binary
                ...
                try {
                    ...
                    if (parsed != Operators::Opers::invalid) {
                        const auto rhs = std::dynamic_pointer_cast<eval::Constant_AST_Node < T>>
                        (node->children[1])->m_value;
        ...
        auto optimize(const eval::AST_Node_Impl_Ptr <T> &node) {
            ...
            if (node->identifier == AST_Node_Type::Prefix
                ...
                try {
                    ...
                    const auto lhs = std::dynamic_pointer_cast<eval::Constant_AST_Node < T>>
                    (node->children[0])->m_value;
                ...
                const auto arg = std::dynamic_pointer_cast<eval::Constant_AST_Node < T>>
                (node->children[1]->children[0])->m_value;
            ...
            if (eq_node->identifier == AST_Node_Type::Equation
                ...
                if (begin.get_type_info().bare_equal(user_type<int>())
                    ...
                    return make_compiled_node(for_node, {body},
                                              [id, start_int, end_int](const std::vector<eval::AST_Node_Impl_Ptr < T>>
                    &children,
